...
$paginacion = new Paginacion('prestamo', 10);
  $paginacion->setOrderBy(array('CodPrestamo','desc'));
  $paginacion->setWhere(array(array('CodUsuario','=',$_SESSION['usuario']['CodUsuario'])));
...
<?php }else{
  echo '<span class="label label-success">Prestamo entregado</span>';
    ...
    //echo '<span class="label label-success">Prestamo entregado</span>';
      $devolucion = Devoluciones::where('CodPrestamo','=',$prestamo->getCodPrestamo());
      $dev = mysql_fetch_row($devolucion);
      echo '<small>Fecha de devolución: </small><span class="label label-info">'.$dev[1].'</span>';
  } 
  ?>
    </td>
    <td> 
...
<?php
  ...
  for ($i=1; $i <= $paginacion->getPages(); $i++) { 
    ...
    echo '<li '.$active.'><a href="index.php?menu=prestamo_listar&pagina='.$i.'" >'.$i.'</a></li>';
  }
?>
</ul>
</section>